...
class Model(object):
    ...
    def add_crossdiff_op(self, aspect_hidden, polarity_hidden, m_hidden, g_hidden_size, max_pooling=True):
        ...
        with tf.variable_scope("crossdiff_op_"):
            ...
            G_aspect_polarity = tf.reshape(G_aspect_polarity, shape=[g_hidden_size, -1])  # (g_hidden_size, m_hidden * g_hidden_size)
            # G_aspect_polarity-->(batch_size, g_hidden_size, m_hidden * g_hidden_size)
            G_aspect_polarity = tf.tile(tf.expand_dims(G_aspect_polarity, axis=0), multiples=tf.stack([tf.shape(aspect_hidden)[0], 1, 1]))
            # aspect_hidden-->(batch_size, max_seq_len, g_hidden_size)
            # shared_hidden_aspect_polarity-->(batch_size, max_seq_len, m_hidden * g_hidden_size)
            ...
            # polarity_hidden_transpose-->(batch_size, g_hidden_size, max_seq_len)
            polarity_hidden_transpose = tf.transpose(polarity_hidden, [0, 2, 1])
            # shared_hidden_aspect_polarity-->(batch_size, max_seq_len * m_hidden, max_seq_len)
            ...
            if max_pooling:
                # shared_hidden_aspect_polarity-->(batch_size, max_seq_len, 1, max_seq_len)
                shared_hidden_aspect_polarity = tf.reduce_max(shared_hidden_aspect_polarity, axis=-2)
            else:
                # shared_hidden_aspect_polarity-->(batch_size, max_seq_len, max_seq_len, m_hidden)
            ...
            # polarity_vector-->(batch_size, max_seq_len, max_seq_len)
    ...
    def add_full_connection_op(self, task_name, inputv, out_size, active_function=None):
        ...
        with tf.variable_scope("proj" + task_name + self.graph_suffix):
            ...
            # output-->(batch_size * max_seq_len, in_size)
            output = tf.reshape(inputv, [-1, in_size])
            # pred-->(batch_size * max_seq_len, out_size)
